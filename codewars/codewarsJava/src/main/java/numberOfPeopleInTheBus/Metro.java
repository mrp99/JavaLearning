...
* empty and some people might still be inside the bus, they are probably sleeping there
...
public class Metro {
    public static int countPassengers(ArrayList<int[]> stops) {
        int peopleInMetro = 0;
        for (int[] stop : stops) {
            peopleInMetro = peopleInMetro + stop[0];
            peopleInMetro = peopleInMetro - stop[1];
        }
        return Math.max(0, peopleInMetro);
    }
}
